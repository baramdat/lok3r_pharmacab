...
class userController extends Controller
{
    ...
    public function users(Request $request){
        ...
        if (isset($user) && sizeof($user)>0){
            ...
            foreach ($user as $usr){
                ...
                }elseif(file_exists("uploads/files/".$usr->photo)){
                    $img='<img src="'.asset('uploads/files/'.$usr->photo).'" alt="img"
                ...
                if(Auth::user()->id ==$usr->id){
                    ...
                    $btun = '<a class="dropdown-item btnEdit" href="/edit/user/'.$usr->id.'"><i class="fe fe-edit me-2"></i> Edit</a>
                ...
                if(Auth::user()->id ==$usr->id){
                    ...
                    <li>
                        <a href="/edit/user/'.$usr->id.'" class="btnEdit bg-success text-white border-success border"
    ...
    public function editUser($id){
        ...
        if ($user){
            ...
            return view('templates/users/user_edit',['user'=>$user]);
        }else{
    ...
    public function updateUser(Request $request){ 
        ...
        $user->first_name = $request->first_name;
        $user->last_name = $request->last_name;
        if (User::where('email',$request->email)->where('email','!=',$user->email)->exists()){
    ...
    public function userDetail($id){
        $user=User::find($id);
        if (! empty($user)){
            return view('templates.users.user_detail',['user'=>$user]);
        }else{
     ...
     public function updateProfile(Request $request){
         ...
         if (User::where('email',$request->email)->where('email','!=',$user->email)->exists()){
            return response()->json(['status'=>'fail','msg'=>'Email Already Exists!']);
             ...
             $user->additional_contact_email=$request->additional_contact_email;
             $user->additional_address=$request->additional_address;
             $user->additional_phone_number=$request->additional_phone_number;
             $user->fedex_account=$request->fedex_account;
