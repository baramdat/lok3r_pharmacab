    ...
    public function add_history(Request $request)
    {
        try {
            ...
            if ($request->product_action == 'sub') {
                ...
                $inventory->save();
                if ($quantity < 10) {
                    $users=User::where('site_id',$inventory->site_id)->get();
                    foreach($users as $user){
                        $name=ucwords($user->first_name). ' '.$user->last_name;
                        Mail::send('templates.email.quantity',['name'=>$name,'product'=>$inventory->inventory_item->name,'site'=>$inventory->site->name], function ($message) use ($user) {
                            $message->to($user->email)
                                ->from(env('MAIL_FROM_ADDRESS'), env('APP_NAME'))
                                ->subject("Quantity update");
                        });
                    }
                }
